...
class FavController: UITableViewController, UITabBarControllerDelegate {

    static let shared = FavController()
...
//        fetchPopularMovies() { success in
//               if success {
//                   self.fetchNowPlayingMovies(){ success in
//                       if success{
//                           self.fetchfavMovies(){ success in
//                               if success {
...
//    }
//
    func fetchfavMovies(completion: @escaping(Bool) -> Void){
        DispatchQueue.global(qos: .background).async {
            API_Request.shared.fetchFavoriteMovies(){ FavMovies, Error in
                DispatchQueue.main.async {
    ...
    func fetchImages() {
        ...
        DispatchQueue.global(qos: .background).async {
           ...
           // let popularMovieResults = self.popularMovies?.results
            let favMoviesResults = self.favMovies?.results
...
//                debugPrint("Result is nil")
//            }
            if let results = favMoviesResults{
                for results in results{
                    group.enter()
                    API_Request.shared.downloadImgfromURL(movieID: results.id, posterPath: results.posterPath){ moviePoster in
                        DispatchQueue.main.async {
...
//            case 3:
                ...
                navigationItem.backBarButtonItem = backButton
                dv.movieId = favMovies?.results[indexPath.row].id ?? nil
//            default:
...
//        case 3:
            return favMovies?.results.count ?? 0
//        default:
//            return 0
//        }
    }
...
//                completion(true)
//            case 3:
                self.favMovies?.results.remove(at: indexPath.row)
                self.tableView.deleteRows(at: [indexPath], with: .automatic)
                completion(true)
//            default:
//                completion(false)
...
//            }
//        case 3:
            cell.movieTitle.text = favMovies?.results[indexPath.row].title ?? "Unable to load data"
            let vote = favMovies?.results[indexPath.row].voteCount ?? 0
            cell.votecount.text = "Votes:\(String(vote))"
            if indexPath.row <= moviePoster.count {
                var posterindex = -1
                if let favMovies = favMovies{
                    for index in 0..<moviePoster.count{
                        if(moviePoster[index].movieId == favMovies.results[indexPath.row].id)
                        {
                            posterindex = index
                            break
                        }
        ...
        cell.switchValueChanged = { isOn in
                if isOn {
                    ...
                    let movieid = self.favMovies?.results[indexPath.row].id ?? 0
