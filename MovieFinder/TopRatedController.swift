...
//        case 2:
            ...
            let vote = topRatedMovies?.results[indexPath.row].voteCount ?? 0
        ...
        cell.fav.isOn = false
        API_Request.shared.checkIfMovieIsFavorited(movieId: self.topRatedMovies?.results[indexPath.row].id ?? 0) { isFavorited in
            if let isFavorited = isFavorited {
                ...
                print("error in checkiffavorite toprated")
            }
        }
//        case 3:
...
//            }
        ...
        cell.switchValueChanged = { isOn in
                if isOn {
                    ...
                    let movieid = self.topRatedMovies?.results[indexPath.row].id ?? 0
