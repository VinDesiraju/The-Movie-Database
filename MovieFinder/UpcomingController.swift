...
//    }
//
    func checkIfMovieIsFavorited(movieId: Int, sessionId: String) {
        let urlString = "https://api.themoviedb.org/3/movie/\(movieId)/account_states?api_key=your_api_key&session_id=\(sessionId)"
            ...
            do {
                ...
                if let json = jsonObject as? [String: Any],
                   let favorite = json["favorite"] as? Bool {
                    if favorite {
                        // the movie is favorited
                        // turn on the switch or perform some other action
                    } else {
                        // the movie is not favorited
                        // turn off the switch or perform some other action
                    }
                }
            } catch {
...
//        case 3:
            ...
            let vote = upcomingMovies?.results[indexPath.row].voteCount ?? 0
        ...
        API_Request.shared.checkIfMovieIsFavorited(movieId: self.upcomingMovies?.results[indexPath.row].id ?? 0) { isFavorited in
            if let isFavorited = isFavorited {
                DispatchQueue.main.async {
                    if isFavorited {
                        cell.turnSwitchOn(val: true)
                        //print("favorite \(self.upcomingMovies?.results[indexPath.row].id ?? 0)")
                    }
                }
                
            } else {
                print("error in checkiffavorite upcoming")
            }
        }
        ...
        cell.switchValueChanged = { isOn in
                if isOn {
                    ...
                    let movieid = self.upcomingMovies?.results[indexPath.row].id ?? 0
